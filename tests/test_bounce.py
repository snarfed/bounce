...
class BounceTest(TestCase, Asserts):
    ...
    @staticmethod
    def make_bridged_copy(orig='http://in.st/users/alice', copy='did:plc:alice',
                          **kwargs):
        """Makes a Bridgy Fed user with a bridged copy in another protocol.

        Args:
          orig (str): ActivityPub actor id of the original user
          copy (str): ATProto DID of ``orig``'s bridged copy
        """
        with ndb.context.Context(bridgy_fed_ndb).use():
            return ActivityPub(id=orig, copies=[Target(protocol='atproto', uri=copy)],
                               **kwargs).put()
...
<span style="unicode-bidi: isolate">@alice@in.st</span>""", body)
    ...
    def test_to_omits_and_logs_out_bridged_copy(self):
        ...
        self.make_bridged_copy(orig='http://in.st/users/bob')
    ...
    def test_review_to_is_bridged_copy(self):
        ...
        self.make_bridged_copy(orig='http://in.st/users/bob', copy='did:plc:bob')
    ...
    def test_review_to_is_from_bridged_copy_logged_in(self):
        """The from account's own bridged copy, but logged in, so it's not the
        "migrate to a new Bluesky PDS" flow."""
        with self.client.session_transaction() as sess:
            from_auth = self.make_mastodon(sess)
    ...
    def test_review_to_is_from_bridged_copy_not_logged_in(self, _):
        """Migrating the from account's bridged copy to a new Bluesky PDS."""
        with self.client.session_transaction() as sess:
            from_auth = self.make_mastodon(sess)
        ...
        self.make_bridged_copy(enabled_protocols=['atproto'])
        ...
        self.assertIn('<form action="/confirm" method="get">',
...
* 🌐 e.ve""", text, ignore_blanks=True)
    ...
    def test_review_task_mastodon_to_bluesky(self):
        ...
        migration = self._test_review_task_activitypub_to_bluesky(
            ...
            follows=[alice, bob, eve, bot])
        self.assertEqual(['did:plc:bob', 'did:plc:eve'], migration.to_follow)
        self.assertEqual([ndb.Key('ActivityPub', 'http://in.st/users/alice')],
    ...
    def test_migrate_task_mastodon_to_bluesky(
        ...
        dormant_key = ndb.Key('ActivityPub', 'http://other/bob')
    ...
    def test_migrate_task_mastodon_to_bluesky_existing_bridged_account_to_new_pds_default_handle(
        ...
        migration = Migration(id='@alice@in.st atproto', from_=from_auth, to=to_auth,
                              to_follow=['did:bob', 'did:eve'],
                              state=State.migrate_follows).put()

        # default ATProto handle on ap.brid.gy
        self.assertEqual('alice.in.st.ap.brid.gy', from_key.get().handle_as(ATProto))

        resp = self.post('/queue/migrate', from_auth, to_auth)
        self.assertEqual(200, resp.status_code)

        # PLC update should use the new PDS handle
        ...
        self.assert_equals(call('https://plc.local/did:plc:alice', json={
                ...
                'alsoKnownAs': ['at://alice.new.pds.com'],
                'services': {
                    'atproto_pds': {
                        'type': 'AtprotoPersonalDataServer',
                        'endpoint': 'https://new.pds.com',
    ...
    def test_migrate_task_mastodon_to_bluesky_existing_bridged_account_to_new_pds(
        ...
        mock_post.assert_has_calls([
            call('https://new.pds.com/xrpc/com.atproto.repo.createRecord', json={
                'repo': 'did:plc:alice',
                'collection': 'app.bsky.graph.follow',
                'record': {
                    '$type': 'app.bsky.graph.follow',
            ...
            call('https://new.pds.com/xrpc/com.atproto.repo.importRepo',
                 data=ANY, json=None, headers=ANY, auth=None),
            call('https://new.pds.com/xrpc/com.atproto.server.activateAccount',
                 data=ANY, json=None, headers=ANY, auth=None),
        ], any_order=True)
