    ...
    def decorator(fn):
        ...
        def wrapper(*args, **kwargs):
            ...
            if to_params:
                ...
                if check_bridged and is_bridged_copy(to_auth):
...
def user_id(auth):
    ...
    Returns:
      str:
    """
    return (auth.actor_id() if AUTH_TO_PROTOCOL[auth.__class__] == ActivityPub
            else auth.key.id())
...
def get_user(auth):
    ...
    with ndb.context.Context(bridgy_fed_ndb).use():
        return proto.get_or_create(user_id(auth), allow_opt_out=True)
